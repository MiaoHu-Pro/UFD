...
import numpy as np
import pandas as pd
from create_description.utilities import Enti, write_to_file, entity_text_process, relation_text_process
from create_description.get_word2vector import get_sentence_init_embedding, word2vector
from create_description.get_word2vector import read_word_bag
...
def get_hrt_description_embedding(_h, _r, _t, entity_res, relation2id):
    ...
    all_entity_res_obj = entity_res['all_entity_description_obj']
    ...
    head_description_list = [all_entity_des_word[i] for i in head_index]  # get head entity description

    tail_description_list = [all_entity_des_word[i] for i in tail_index]  # get tail entity
    ...
    for i in range(len(relation_name)):
        rel_des = str(relation_name[i]) + ', ' + 'which is between ' + head_obj[i].symb + ' and ' + tail_obj[
            i].symb + ';' \
